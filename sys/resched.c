...
int	resched()
{
	...
	register int i;
	unsigned int pp,qq,store,pageth;
	pd_t *pde;
	pt_t *pte;
	...
	/* write old process' pages to shared BS for new process to access */
	for(i=0; i < NFRAMES; i++){
			...
			get_virt_addr(vaddr, &virtaddr);        
		        pp = virtaddr.pd_offset;
		        qq = virtaddr.pt_offset; 

		        pdbr = proctab[currpid].pdbr;
		        pt = pdbr + sizeof(pd_t) * pp;   
		        pde = (pd_t *) pt;                                              /* pde points to pth page table */
		        pte = (pt_t *) (pde->pd_base * NBPG + sizeof(pt_t) * qq );       

			/* if the page is dirty - write to the BS */
		        if(pte->pt_dirty){
				if(bsm_lookup(currpid, vaddr, &store, &pageth) == SYSERR){				
                        		kprintf("Backing Store lookup failed! Killing process %d!\n",currpid);
		                        kill(currpid);
                		}
	                	if(bsm_tab[store].bs_ispriv == BSM_SHARED)
	...
	/* read latest pages from the BS for new process */
	for(i=0; i <NFRAMES; i++){
	...
	PrintSaved(nptr);
#endif
	write_cr3(nptr->pdbr);	/* placed before the actual context switch because the subroutine ends up in the context of a new process */

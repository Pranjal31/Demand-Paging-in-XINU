...
bs_map_t bsm_tab[NBS]; 		/* bsm table 				*/
fr_map_t frm_tab[NFRAMES];	/* inverted page table or frame table	*/
...
struct 	fifo_node * fifohead;	/* head of fifo queue			*/
int 	num_sc;			/* number of pages in sc queue		*/
int 	sc_curr;		/* index of current frame in SC circular queue */
char 	vers[80];
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i=0; i < 4; i++){
		...
		frm_tab[frm].fr_pid = NULLPROC;
		frm_tab[frm].fr_refcnt = NPTE;
		pte = (pt_t *)((FRAME0 + frm) * NBPG); 	/* physical start address of frm that will hold the PT */
		for(j=0; j < NPTE; j++){
			pte->pt_pres = 1;	/* page is present for Global Page Table */
			pte->pt_write = 1;
			pte->pt_user = 0;
			pte->pt_pwt = 0;
			pte->pt_pcd = 0;
			...
			pte->pt_global = 0;
			pte->pt_avail = 0;
			pte->pt_base = i * FRAME0 + j;	/* page starts at (pte->pt_base * 4096) */
			pte++;		/* next PT entry */
		}
	}

	/* allocate a Page Directory for Null Proc */
	allocate_pd(NULLPROC, &pdfrm);
	
	/* initialize the Page Directory */
	initialize_pd(pdfrm);

	/* pdbr points to the page directory */
	pptr->pdbr = (FRAME0 + pdfrm) * NBPG ;
	write_cr3(pptr->pdbr);
